...
import { getShelfList } from '../services/api.services';
...
class ShelfList extends Component {
    ...
    componentDidMount() {
        getShelfList()
            .then( shelfList => this.setState({ shelfList }))
            .catch( err => {
    ...
    render() {
        const shelfList = this.state.shelfList.map( shelf =>
            <Link to={`/shelf/${shelf}`} className='sl-Shelf_Link' key={getUId()}>
                <p> Shelf {shelf} </p>
            </Link>
        );
